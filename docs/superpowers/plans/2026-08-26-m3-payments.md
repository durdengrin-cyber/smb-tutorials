...
**Cross-cutting change, easy to miss:** `effectiveStatus` gains `payment_deadline` in its row type. **Every `.select()` that feeds it must add that column**, or TypeScript passes and the rule silently stops firing. The call sites are: `dashboard/actions.ts` (two queries), `teachers/actions.ts`, `waiting/[sessionId]/page.tsx`, `call/[sessionId]/page.tsx`, `dashboard/session-history.tsx` (two queries). Task 1 changes the type; Task 7 fixes every call site; Task 13 greps to prove none were missed.
...
Run: `npx tsc --noEmit` — **expect FAILURES** in the six call sites listed in the File Structure note. That is correct and expected at this task; Task 7 fixes them. Do not fix them here.
...
**Files:**
- Create: `src/lib/payments/port.ts`, `src/lib/payments/stub.ts`, `src/lib/payments/index.ts`
...
Run: `npx eslint src/lib/payments` — expect clean.

- [ ] **Step 7: Commit**

```bash
git add src/lib/payments
git commit -m "feat: payment port with a development stub adapter"
```
...
export async function verifyPaymentNow(sessionId: string): Promise<void> {
  ...
  // Deliberately minimal for M3: the provider adapter gains a `fetchPayment`
  // method in Task 12 and this becomes a real reconciliation call. Until then
  // it records the attempt so an outage is visible in logs rather than silent.
  console.info(
...
- Modify: `src/app/teachers/actions.ts`, `src/app/waiting/[sessionId]/page.tsx`, `src/app/call/[sessionId]/page.tsx`, `src/app/dashboard/session-history.tsx` — add `payment_deadline` to every `.select()` feeding `effectiveStatus`
...
- [ ] **Step 2: Add `payment_deadline` to every other `effectiveStatus` call site**

Add the column to the `.select()` string in each of:
- `src/app/teachers/actions.ts` — the `openRows` query, and change `.in("status", ["pending","active"])` to `.in("status", ["pending","accepted","paid","active"])`
- `src/app/waiting/[sessionId]/page.tsx`
- `src/app/call/[sessionId]/page.tsx`
- `src/app/dashboard/session-history.tsx` — both queries

- [ ] **Step 3: Verify the whole tree type-checks again**

Run: `npx tsc --noEmit`
Expected: **exit 0.** This is the first task since Task 1 where it should be clean; if it is not, a call site was missed.
...
git add src/app/dashboard/actions.ts src/app/teachers/actions.ts \
  "src/app/waiting/[sessionId]/page.tsx" "src/app/call/[sessionId]/page.tsx" \
