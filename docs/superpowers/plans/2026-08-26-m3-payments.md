...
`payment_provider` is written here alongside the ref and the URL: they are one logical fact — which adapter opened which charge — and splitting them across two authorities made the statement unexecutable by anyone (review finding I1).
...
**Two defects Task 7 exposed in this file must be fixed here — they are not
optional polish, and one of them strands a teacher today:**

1. **`busy` is never reset on the success path.** `accept()` sets `busy = true`,
   awaits `acceptSession`, and only calls `setBusy(false)` inside the error
   branch. That was correct while success meant `redirect()` threw and unmounted
   the component. Task 7 removed the redirect, so the component now stays
   mounted with `busy` stuck `true` — and neither realtime handler resets it.
   The card vanishes (the UPDATE handler clears it), but **the next incoming
   request that teacher receives in the same page session renders with both
   buttons permanently disabled**, no error shown, recoverable only by a full
   reload. Whatever shape the new waiting state takes, `busy` must be reset on
   every path that leaves the accept in flight.
2. **The comment at the accept handler is now false.** `// On success this
   redirects and never returns a value.` — it does not redirect. That stale
   assumption is precisely what left the `busy` bug dormant; delete or correct
   it.
...
**Provider chosen: Razorpay** (decision recorded in spec §11). Still blocked
until the user supplies **test-mode** keys — `RAZORPAY_KEY_ID`,
`RAZORPAY_KEY_SECRET` and a webhook secret. Test mode is enough for this task
and Task 13; live keys matter only when real money moves.

**Concrete shape, verified against Razorpay's docs — but verify again as you
build, because these are the details that go stale:**

- **`createCheckout`** → `POST /v1/payment_links` with `amount` (paise),
  `currency: "INR"`, `callback_url` = our success URL, `callback_method: "get"`,
  and `reference_id` = the session id. Take `short_url` as `checkoutUrl` and the
  link `id` as `paymentRef`. Payment Links need no pre-created Order, which is
  why they fit our redirect model better than Standard or Hosted Checkout.
- **`verifyWebhook`** → HMAC-SHA256 of the **raw body** with the webhook secret,
  compared against the `x-razorpay-signature` header, timing-safe. Razorpay's
  docs say explicitly: *do not parse or cast the webhook request body.* Our
  handler already reads `req.text()`; do not change that.
- **Event mapping** → a `payment.captured` (or `payment_link.paid`) event
  becomes `kind: "succeeded"`; map the amount from paise directly, and carry
  `x-razorpay-event-id` into your logging so a redelivery is identifiable.
- **`refund`** → `POST /v1/payments/{payment_id}/refund`. Note the refund is
  against the **payment id**, which is not the payment-link id — resolve it from
  the webhook payload and make sure `payment_ref` stores whatever you will need
  later to refund. If those are different identifiers, say so in your report:
  it affects what `createCheckout` should stamp.

That last point is the one most likely to bite. Our schema stores a single
`payment_ref`, written at checkout creation and write-once at the database
level. If refunding needs an identifier only known *after* payment, raise it
rather than working around it — it is a schema question, not an adapter one.
...
- [ ] **Step 2: Add the env vars** — `PAYMENT_PROVIDER=razorpay`, `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, `PAYMENT_WEBHOOK_SECRET` — to `.env.local` and to Vercel for Production, Preview and Development. Prefer plain `fetch` over the SDK: `daily.ts` established the injected-`fetchImpl` pattern this codebase tests against, and Razorpay's REST surface here is three calls.
- [ ] **Step 3: Implement the three port methods** against the provider's API, taking `fetchImpl` like `src/lib/daily.ts` does.
- [ ] **Step 3b: Add a fourth method, `fetchPayment(paymentRef): Promise<WebhookEvent | null>`, and make `verifyPaymentNow` real.**

  **Two findings from Task 6's review are carried here, because they only become
  consequential when this function gains teeth:**

  1. **Destructure the `error` on its session read.** Today it takes only `data`,
     so a genuine query failure and "no such row" are the same silent no-op.
     That is tolerable while the function drives nothing; once it drives the
     same transition the webhook drives, a swallowed read error means a stuck
     payment gets no second-path attempt **and leaves no trace of why** —
     defeating the one job §3.6 gives it. Match the webhook's pattern: log the
     cause, distinguish absent from failed.
  2. **Move the incompleteness caveat, or delete it.** It currently sits inside
     the body, past three early returns, while the *leading* comment describes
     the dual path as though it were live. A caller who reads the name and the
     leading comment — the exact reader most likely to be misled — would
     conclude the re-check already happens. When this step makes it real, the
     leading comment must describe what it actually does. Task 6 shipped only the hook and a log line, because with a stub there is no provider to query. The spec's §3.6 dual path is not actually complete until this exists: add the method to `PaymentPort`, have `verifyPaymentNow` call it and drive the same idempotent transition the webhook drives. **Without this step the milestone has one path to confirmation, not two.**
...
- [ ] **Step 3b: The two behaviours added after Task 11 that ONLY a browser can prove.** Both are client-side realtime; types, lint and the build say nothing about either.

  **Presence follows the commitment, not the navigation** (spec §9's regression, root fix):
  1. Teacher goes Available. A student on `/teachers` sees their card.
  2. Teacher accepts a request from a *second* browser. Within a moment the first student's list must **drop the card** — the teacher is now "In a session" (amber) on their own dashboard, not "Offline", and the toggle still reads as online.
  3. Let the payment window expire. The teacher must return to "Available now" **on their own**, with no toggling, and the card must reappear in the student's list.
  4. Repeat, but pay instead. The teacher lands in the call; on returning to `/dashboard` they are available again (the localStorage intent path, unchanged).

  **Cancel inside the payment window** (spec §3.1's `accepted -> cancelled`):
  5. Teacher accepts; the student's screen shows Pay **and Cancel**. Click Cancel: the student returns to their filtered list, and the teacher's card clears and they go available again immediately — not after the full 120 seconds.
  6. The race, worth one deliberate attempt: complete the payment and click Cancel at the same moment. Exactly one of two outcomes is acceptable — the student lands in the call, or the session cancels and **the payment is refunded** (check the provider dashboard). The student must never be walked back to the list while holding a paid session; if that happens, stop, because it is a lost payment.

  **A second request while busy** — with presence fixed this should be hard to reach, so force it: with the teacher mid-payment-window, point a second student straight at `/waiting/...` for a new request (or send one before the list refreshes). The teacher's dashboard must keep showing **their own in-flight session**, not the newer request.
...
- [ ] **Step 8b: Prove the DEPLOYED webhook is really configured — an unsigned curl cannot tell you**

```bash
node scripts/probe-deployed-webhook.mjs https://smb-tutorials.vercel.app/api/payments/webhook
```

Expects **signed → 200, wrong signature → 400**. Both 400 means the signature never verified: a credential missing or misnamed on Vercel, a deployed `PAYMENT_WEBHOOK_SECRET` that differs from Razorpay's, or a deployment that predates the variables.

**Why a plain unsigned curl is not enough** (learned 2026-08-27): `getPaymentPort()` throws at construction when a credential is missing, and that throw is caught by the *same* `try/catch` that rejects a bad signature — so a fully unconfigured deployment answers `400 bad signature`, identical to a correctly configured one refusing a forgery. Only a correctly signed request separates them. Remember also that `NEXT_PUBLIC_` variables are inlined at **build** time, so adding an env var does nothing until the next deployment.
