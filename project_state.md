...
5. **M3 is 12 of 13 tasks complete** (Tasks 1-12). Payments work end to end against a development stub. **Task 11's owed fix round is done** — all three review findings closed and re-verified live on 2026-08-27; see "Task 11, closed" below. What remains, in order:
   ...
   - **⚠ FIRST: add the four payment env vars to Vercel** (`PAYMENT_PROVIDER=razorpay`, `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, `PAYMENT_WEBHOOK_SECRET`) for Production, Preview and Development — Task 12 Step 2's second half, still outstanding. `.env.local` is done. Without these the preview deployment falls to the stub, which refuses to run in a production build, so **no deployed checkout will work until this is done**.
